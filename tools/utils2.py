...
class Box3D_inter(object):
    ...
    def __init__(self, data):
        ...
        # extract 3d bounding box information
        self.h = data[0]  # box height
        self.w = data[1]  # box width
...
def project_cam2_to_velo(calib):
    ...
    P2 = np.vstack((calib['P2'].reshape(3, 4), np.array([0., 0., 0., 1.])))
    P2_ref2rect_inv = np.linalg.inv(P2) 
...
def draw_projected_box3d(image, qs, color=(0, 255, 0), thickness=1):
    ...
    if qs.any() < 0:
        ...
        for k in range(0, 4):
            ...
            i, j = k, (k + 1) % 4
            if qs[i, 0] * qs[j, 0] >= 0  and qs[i, 1] * qs[j, 1] >= 0:
            ...
            if qs[i, 0] * qs[j, 0] >= 0  and qs[i, 1] * qs[j, 1] >= 0:
                cv2.line(image, (qs[i, 0], qs[i, 1]), (qs[j, 0], qs[j, 1]), color, thickness, cv2.LINE_AA)
        return image
    # if 0 <= (min(qs[:,0]) + max(qs[:,0]))/2 < img_width  and 0 <= (min(qs[:,1]) + max(qs[:,1]))/2 < img_height:
    #     x_min = max(min(qs[:,0]), 0)
    #     x_max = min(max(qs[:,0]), img_width-1)
    #     y_min = max(min(qs[:,1]), 0)
    #     y_max = min(max(qs[:,1]), img_height-1)
    #     cv2.rectangle(image, (x_min, y_min), (x_max, y_max), (0, 0, 255), thickness, cv2.LINE_AA)
